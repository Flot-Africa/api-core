...
/*
    ...
    public void repayLoan(UUID loanId, RepaymentRequest request) {
...
*/
    ...
    public void repayLoanTest(RepaymentRequest request) {
        Map<String, String> headers = headersProvider.getDefaultHeaders();
        int i = 0;
        // 1. Créer un PaymentIntent
        Map<String, Object> intentPayload = new HashMap<>();
        intentPayload.put("customerReference", "driver_"+0+i);
        intentPayload.put("purchaseReference", "loan_" + i + "_" + System.currentTimeMillis());
        intentPayload.put("amount", request.amount());
        intentPayload.put("currency", "XOF");

        var intent = hub2Client.createPaymentIntent(intentPayload);
        i++;
        // 2. Tenter un paiement Mobile Money
        Map<String, Object> paymentPayload = new HashMap<>();
        paymentPayload.put("token", intent.token());
